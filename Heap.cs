...
namespace Heap
{
    public class Heap<K, D> where K : IComparable<K>
    {
        ...
        private void DownHeap(int start)
        {
            ...
            while ((data[rightChild] !=null) && (data[rightChild] != null))
            {
                if ((comparer.Compare(data[position].Key, data[rightChild].Key) < 1) || (comparer.Compare(data[position].Key, data[leftChild].Key) < 1))
                {
                    if (comparer.Compare(data[leftChild].Key, data[rightChild].Key) < 0)
                    {
                        minChild = leftChild;
                    }
